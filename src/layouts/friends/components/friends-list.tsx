		...
		return (
			<div className="space-y-1">
				{Array.from({ length: 5 }).map((_, i) => (
					<div
						key={i}
						className="flex items-center justify-between p-3 border rounded-xl border-subtle bg-subtle animate-pulse"
					>
	...
	return (
		...
		>
			{allFriends.map((friend) => (
				<div
					key={`f-${friend.id}`}
					className="flex items-center justify-between p-3 transition-all duration-200 border rounded-xl border-content hover:elevation-sm"
				>
