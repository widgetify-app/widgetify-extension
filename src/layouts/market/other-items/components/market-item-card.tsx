...
export function MarketItemCard({
	...
	return (
		<div className="flex flex-col overflow-hidden transition-all duration-200 border bg-widget rounded-2xl border-faint hover:border-primary/30 hover:elevation-sm group">
			{/* Preview area */}
			<div className="relative overflow-hidden bg-subtle flex-shrink-0 min-h-[80px]">
				<RenderPreview
					item={item}
					handlePreviewClick={() => {
				...
				<div className="absolute top-2 right-2">
					<span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md bg-subtle backdrop-blur-sm border border-faint text-[9px] text-subtle font-medium">
				...
				{!canPreview && (
					<button
						...
						className="absolute bottom-1.5 left-1.5 flex items-center gap-1 px-2 py-1 rounded-lg bg-widget border border-subtle text-muted hover:text-primary transition-colors text-[10px] font-medium backdrop-blur-sm cursor-pointer opacity-0 group-hover:opacity-100"
					>
