...
export function ArzLiveLayout() {
    ...
    const DEFAULT_VISIBLE_ITEMS = { sm: 4, md: 6 } // Different values for breakpoints
    const totalItems = selectedCurrencies.length + 1
    const showExpandButton = totalItems > DEFAULT_VISIBLE_ITEMS.md

    const containerVariants = {
        collapsed: { 
            height: `${ITEM_HEIGHT * DEFAULT_VISIBLE_ITEMS.sm}px`,
            '@media (min-width: 640px)': {
                height: `${ITEM_HEIGHT * DEFAULT_VISIBLE_ITEMS.md}px`,
            }
        },
    ...
    return (
        <div className="relative">
            <motion.div
                ...
                initial="collapsed"
                className={`flex flex-col gap-0.5 sm:gap-1 px-1 sm:px-2 py-1 sm:py-2 rounded-lg sm:rounded-2xl min-h-80 sm:min-h-64 ${themeUtils.getCardBackground()}`}
            ...
            {showExpandButton && (
                <motion.button
                    ...
                    className={`absolute p-0.5 sm:p-1 transition-colors translate-x-1/2 rounded-full bottom-1 right-1/2 ${getExpandButtonStyle()}`}
