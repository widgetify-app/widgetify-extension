...
export function BookmarksComponent() {
  ...
  const isManageable = (bookmark: Bookmark) => {
    if (
      "isManageable" in bookmark &&
      typeof bookmark.isManageable === "boolean"
    ) {
          ...
          bookmark ? (
            <div
              ...
              className={`transition-transform duration-200 ${
                dragOverIndex === i
                  ? "scale-110 border-2 border-blue-400 rounded-xl"
                  : ""
              }`}
            >
              <BookmarkItem
                bookmark={bookmark}
                onMouseDown={(e) => handleBookmarkClick(bookmark, e)} //fix: enable middle-click to open bookmarks when widget is scrollable by adding onMousedown event for middle click
                onClick={(e) => {
                  if (e?.button === 0) handleBookmarkClick(bookmark, e); // only handle left click here
                }}
                canAdd={true}
