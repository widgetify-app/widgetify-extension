...
export function useBasePetLogic({
  ...
  const [behaviorState, setBehaviorState] = useState<PetBehavior>(
    PetBehavior.RESTING,
  )
  ...
  const movePet = useCallback(
    (currentPosition: Position, currentDirection: number) => {
      ...
      const newY =
        currentPosition.y > 0 ? Math.max(0, currentPosition.y - 0.5) : 0
  ...
  const moveToTarget = useCallback(
    (currentPosition: Position) => {
      ...
      const speed =
        action === 'run' ? dimensions.runSpeed : dimensions.walkSpeed
