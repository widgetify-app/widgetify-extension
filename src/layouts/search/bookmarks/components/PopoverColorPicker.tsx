...
const PopoverColorPicker: React.FC<PopoverColorPickerProps> = ({ color, onChange }) => {
  ...
  // Update position when trigger element position changes or when opened
  useEffect(() => {
    if (isOpen && triggerRef.current) {
      const rect = triggerRef.current.getBoundingClientRect();
      setPosition({
        top: rect.bottom + window.scrollY,
        left: rect.right - 20 + window.scrollX, // 200px is the width of the color picker
      });
    }
  }, [isOpen]);
  ...
  // Use color if provided, otherwise default to black
