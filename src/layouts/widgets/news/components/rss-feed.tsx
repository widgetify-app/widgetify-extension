...
export function RssFeedComponent({ url, sourceName, label }: Prop) {
	...
	if (isError) {
		return (
			<div className="flex items-center justify-between gap-2 p-2 border rounded-2xl border-subtle bg-raised">
				...
				<button
					type="button"
					onClick={() => refetch()}
					className="px-2 py-0.5 text-[10px] font-bold rounded-lg cursor-pointer shrink-0 text-content bg-raised transition-ui hover:bg-hovered focus-visible:focus-ring"
				>
					تلاش دوباره
				</button>
			</div>
