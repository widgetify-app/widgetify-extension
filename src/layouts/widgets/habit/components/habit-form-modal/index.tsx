...
import { HABIT_QUICK_PRESETS, type HabitPresetItem } from './constants'
...
export const HabitFormModal: React.FC<HabitFormModalProps> = ({
	...
	const handleSubmit = async () => {
		...
		const [error] = await safeAwait(
			habit ? updateHabit({ id: habit.id, input: payload }) : addHabit(payload)
		)
		...
		>
			<div className="flex flex-col gap-4 text-right select-none">
				...
				<div className="flex flex-col gap-1.5">
					...
					<div className="flex items-center px-3 py-2 transition-colors border rounded-2xl border-content bg-raised focus-within:border-primary">
						<div
						...
						<TextInput
							...
							className="flex-1 text-sm font-medium bg-transparent border-none shadow-none text-content placeholder:text-subtle focus:outline-none focus:ring-0"
						/>
					</div>
				</div>
