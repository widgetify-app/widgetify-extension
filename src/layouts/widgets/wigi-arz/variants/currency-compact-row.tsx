...
interface CurrencyItemCompactProps {
	code: string
}

function CurrencyItemCompact({ code }: CurrencyItemCompactProps) {
...
interface CurrencyCompactRowProps {
	currencies: string[]
}

export function CurrencyCompactRow({ currencies }: CurrencyCompactRowProps) {
	const displayCurrencies = currencies.slice(0, 2)

	return (
		<div className="grid grid-cols-2 gap-1.5 h-full w-full select-none overflow-hidden">
			{displayCurrencies.map((code) => (
				<CurrencyItemCompact key={code} code={code} />
			))}
		</div>
	)
}
