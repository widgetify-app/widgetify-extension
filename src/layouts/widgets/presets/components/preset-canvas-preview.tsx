...
export const PresetCanvasPreview: React.FC<PresetCanvasPreviewProps> = ({
	...
	return (
		<div
			...
			aria-hidden="true"
			className={cn(
				'w-full h-36 p-2 overflow-hidden select-none rounded-xl bg-raised border border-subtle transition-ui',
				className
			)}
		>
			<div
				...
				{resolvedWidgets.map((widget) => {
					...
					return (
						<div
							...
							className="flex items-center justify-center overflow-hidden rounded-md bg-raised text-muted group-hover:bg-brand-subtle group-hover:text-primary transition-ui"
						>
							<Icon
