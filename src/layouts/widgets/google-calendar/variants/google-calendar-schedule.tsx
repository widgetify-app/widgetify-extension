...
const navButtonClass =
	'flex items-center justify-center w-6 h-6 rounded-lg cursor-pointer transition-ui text-muted opacity-70 hover:opacity-100 hover:bg-muted focus-visible:focus-ring'
				...
				{isLoading && (
					<div aria-hidden="true" className="space-y-1.5">
						{Array.from({ length: 3 }).map((_, i) => (
							<div
								key={`loading-item-${i}`}
								className="flex items-center gap-2 p-2 rounded-xl bg-subtle animate-pulse"
							>
								<div className="w-10 h-8 rounded-lg bg-content shrink-0" />
								<div className="flex-1 space-y-1">
									<div className="w-3/4 h-3 rounded bg-content" />
									<div className="w-1/2 h-2 rounded bg-subtle" />
								</div>
							</div>
						))}
					</div>
