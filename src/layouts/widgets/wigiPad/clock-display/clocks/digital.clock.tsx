...
export function DigitalClock({ time, isDayTime, timezone, setting }: DigitalClockProps) {
	...
	return (
		<div className="relative flex flex-col items-center">
				<svg width="120" height="160" viewBox='-1 -1 123 164' className='w-full h-[9rem]'>
					{[...Array(60)].map((_, i) => {
						...
						return (
							<line
								...
								opacity={intSeconds - i >= 0 ? (60 - (Math.abs(intSeconds - 1 - i))) / 60 : ((i - intSeconds + 1)) / 60}
			...
			<div
				className={`${textColor} absolute top-1/2 inset-x-5 w-[calc(100%-2.5rem)] -translate-y-[calc(50%+0.65rem)] flex flex-col items-center text-center text-[2.75rem] leading-none drop-shadow-md font-sans font-black z-10 transition-all duration-300`}
			>
				<span>{hours}</span>
				<span>{minutes}</span>
			</div>
			{setting.showTimeZone && (
				<div className={`${textColor} absolute bottom-[1.4rem] inset-x-5 w-[calc(100%-2.5rem)] text-xs leading-none font-medium tracking-[0.05em] text-center truncate opacity-90 transition-all duration-300`}
				>
