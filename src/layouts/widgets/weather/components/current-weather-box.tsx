...
export const CurrentWeatherBox: React.FC<CurrentWeatherBoxProps> = ({
	...
	return (
		<>
			<header
				className={`relative p-2 overflow-hidden bg-raised border ${banner ? 'border-r-0' : ''} rounded-2xl border-content min-h-28 max-h-28`}
			>
				{banner ? (
					<div
						aria-hidden="true"
				...
				<div className="relative z-10 flex items-center justify-between py-1">
					...
					{weather?.icon?.url ? (
						...
						<div
							aria-hidden="true"
							className="w-20 h-20 rounded-lg animate-pulse bg-raised"
						/>
					)}
				</div>
			</header>

			<div className="p-2 border rounded-2xl bg-raised border-content">
