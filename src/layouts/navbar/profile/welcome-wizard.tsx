...
const StepImage = ({ src }: { src: string; alt: string }) => {
	return (
		<div className="relative flex items-center justify-center w-full overflow-hidden bg-subtle md:w-1/2 min-h-80 rounded-2xl group">
			<div
					...
					<StepWrapper>
						<div className="flex flex-col items-center justify-center w-full p-8 text-center md:w-1/2 md:p-12">
							...
							<Button
								size="sm"
								onClick={nextStep}
								className="w-full h-12 text-base font-bold elevation-lg"
								color={'primary'}
								rounded={'2xl'}
							>
								بزن بریم
		...
		<Modal isOpen={isOpen} onClose={onClose} size="xl" direction="rtl" title=" ">
			<div className="relative overflow-hidden rounded bg-widget">
				{currentStep > 1 && currentStep < totalSteps && (
					<button
						onClick={prevStep}
						className="absolute z-20 p-2 transition-colors rounded-full top-10 right-96 bg-subtle text-content hover:bg-raised"
					>
