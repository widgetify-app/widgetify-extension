...
export function WebAppAuthGate({ scopes, onConfirm }: Prop) {
	...
	return (
		<div className="absolute inset-0 flex items-center justify-center p-6">
			<div className="flex flex-col items-center w-full max-w-sm gap-6 p-6 text-center rounded-2xl bg-content">
						...
						<div className="flex flex-col items-center gap-3">
							...
							<ul className="flex flex-col w-full max-w-xs gap-2 text-xs">
								{scopes.map((s) => (
									<li
										key={s}
										className="px-3 py-2 rounded-lg bg-subtle text-content"
									>
										{SCOPES_LABEL[s] || s}
									</li>
								))}
