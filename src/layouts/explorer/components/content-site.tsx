...
export function RenderContentSite({ link }: SiteProp) {
	...
	return (
		<a
			...
			className={`relative flex flex-col items-center gap-1.5 p-2 rounded-xl transition-all duration-200 group active:scale-95 ${pos} hover:bg-muted border border-transparent hover:border-subtle`}
			style={{
		...
		>
			...
			{badge && (
				<span
					className="absolute top-1 -left-1 rounded-r-md text-center z-20 truncate px-1.5 py-0.2 text-[9px] font-medium max-w-20 border border-(--over-image-border) elevation-sm"
					style={{
			...
			<div className="flex items-center justify-center w-10 h-10 transition-transform duration-200 rounded-xl bg-subtle group-hover:scale-105 group-hover:bg-content">
				<img
					src={link.icon}
