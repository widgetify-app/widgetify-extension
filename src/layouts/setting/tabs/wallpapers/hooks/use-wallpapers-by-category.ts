...
export function useWallpapersByCategory() {
  ...
  const shouldFetchWallpapers =
    !wallpaperCacheRef.current[selectedCategoryId || '']
  const {
  ...
  const getCategoryWallpapers = (categoryId: string) => {
    ...
    const allWallpapers =
      wallpaperCacheRef.current[''] || fetchedWallpapers || []
  ...
  const isLoading =
    categoriesLoading || (wallpapersLoading && shouldFetchWallpapers)
