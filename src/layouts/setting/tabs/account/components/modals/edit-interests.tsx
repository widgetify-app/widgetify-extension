		...
		>
			<SectionPanel title="به چی علاقه داری؟" size="xs">
				<InterestsSelector
					...
					triggerElement={
						<div className="flex flex-wrap w-full min-h-24 max-h-24 h-24 overflow-y-auto gap-1 p-2 transition-all border   border-content rounded-2xl hover:border-brand-subtle! cursor-pointer">
							{interests.length > 0 ? (
								interests.map((id) => (
									<Chip
										key={id}
